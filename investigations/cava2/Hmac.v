...
Section Var.
  ...
  Definition REG_INTR_STATE := Constant (val_of hmac_register 0).
  Definition REG_CMD := Constant (val_of hmac_register 5).
  ...
  Definition hmac_top : Circuit _ [tl_h2d_t] tl_d2h_t := {{
    ...
    let/delay '(digest_buffer, tl_o; registers) :=
      ...
      let fifo_write := write_address >= `Constant (val_of (BitVec _) 2048)` in
      ...
      let cmd_start := write_en && aligned_address == `REG_CMD` && write_data == `Constant (val_of (BitVec _) 1)` && !fifo_write in
      (* cmd_process is set when host writes to hmac.CMD.hash_process *)
      (* and signifies the end of a message *)
      let cmd_process := write_en && aligned_address == `REG_CMD` && write_data == `Constant (val_of (BitVec _) 2)` && !fifo_write in
